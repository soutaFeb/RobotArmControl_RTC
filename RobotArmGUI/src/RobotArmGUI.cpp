    ...
    // </rtc-template>
{
    ...
    RobotArmGUI::m_showAutoButton = 0;
    RobotArmGUI::m_auxiliaryLineNum = 0;
    RobotArmGUI::m_auxiliaryArcNum = 0;
...
RTC::ReturnCode_t RobotArmGUI::onExecute(RTC::UniqueId ec_id)
{
    ...
    if (autoMouseOn) {
        if (GetMouseInput() == 1 && autoClickFlag) {
            autoPush = true;
            autoClickFlag = false;
        }
        if (GetMouseInput() == 0 && autoPush) {
        ...
        if (m_autoSignal_output.data == 0) {
            ...
            if (autoStatus > 3)autoStatus = 3;
        }
    }
    else if (!autoPush && m_autoSignal_output.data == 0)autoStatus = 1;
    if (GetMouseInput() == 1)autoClickFlag = false;
    if (GetMouseInput() == 0) {
        autoClickFlag = true;
        autoPush = false;
    }
    ...
    m_angle_output.data[0] = int(angU * 180 / 3.1415);
    m_angle_output.data[1] = int(angL * 180 / 3.1415);
    m_angle_output.data[2] = int(angS * 180 / 3.1415);
    m_angle_outputOut.write();


    //手先座標表示
    if (m_showCoordinates > 0) {
    ...
    if (m_showSideView == 1) {
        ...
        DrawCircleAA(float(side0x), float(side0y), float(jointSize * 0.5), 32, lineColor, true);
        DrawCircleAA(float(side1x), float(side1y), float(jointSize * 0.5), 32, lineColor, true);
        DrawCircleAA(float(side2x), float(side2y), float(jointSize * 0.5), 32, accentColor, true);
    ...
    for (int i = 1; i <= m_auxiliaryLineNum; i++) {//補助線
        double lineEdgeX = windowWidth * 0.5 + arcRadius * cos(i * PI / (1.0 + m_auxiliaryLineNum));
        double lineEdgeY = windowHeight - (arcRadius * sin(i * PI / (1.0 + m_auxiliaryLineNum)));
        DrawLineAA(float(windowWidth * 0.5), float(windowHeight), float(lineEdgeX), float(lineEdgeY), lineColor);
    }
    for (int i = 1; i <= m_auxiliaryArcNum; i++) {//補助円
        DrawCircleAA(float(windowWidth * 0.5), float(windowHeight), float(i * arcRadius / (1 + m_auxiliaryArcNum)), 128, lineColor, false);
    }
    ...
    DrawCircleAA(float(p2x), float(p2y), float(jointSize * 0.5), 32, accentColor, true);//現在の手先位置の円
    DrawCircleAA(float(p1x), float(p1y), float(jointSize * 0.5), 32, lineColor, true);
    DrawCircleAA(float(p0x), float(p0y), float(jointSize * 0.5), 32, lineColor, true);
