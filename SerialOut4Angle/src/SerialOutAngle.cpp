...
RTC::ReturnCode_t SerialOutAngle::onInitialize()
{
  ...
  std::cout << "SerialOut4Angle ready!" << std::endl;
...
RTC::ReturnCode_t SerialOutAngle::onExecute(RTC::UniqueId ec_id)
{
	...
	if (m_angle_inputIn.isNew()) {
		...
		angHand_upper = int(m_angle_input.data[3] / 10);
		...
		angHand_lower = int(m_angle_input.data[3] * 10) % 100;
		...
		for (int i = 0; i < 8; i++) {
			...
			case 6:data = angHand_upper; break;
			case 7:data = angHand_lower; break;
			}
